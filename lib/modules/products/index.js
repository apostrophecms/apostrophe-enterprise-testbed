...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    self.addTask('distinct-count-tags', 'Makes sure we can get a distinct count for tags on products.', async (apos, argv) => {
      const req = apos.tasks.getReq({ locale: 'en-draft' });
      const results = await self.find(req, {}).distinctCounts(true).toDistinct('tags');
      console.log(results);
    });
